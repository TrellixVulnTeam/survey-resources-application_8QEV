...
*/
import{glsl as o}from"../../../core/shaderModules/interfaces.js";import{DoublePrecision as e}from"../../../core/shaderLibrary/util/DoublePrecision.glsl.js";function t(t,n){const r=t.vertex;r.uniforms.add("uDistanceFalloffFactor","float"),r.code.add(o`
    float distanceBasedPerspectiveFactor(float distance) {
      return clamp(sqrt(uDistanceFalloffFactor / distance), 0.0, 1.0);
    }
  `),r.uniforms.add("uComponentDataTex","sampler2D"),r.uniforms.add("uComponentDataTexInvDim","vec2"),t.attributes.add("componentIndex","float"),r.constants.add("componentColorFieldOffset","float",0),r.constants.add("componentOtherFieldOffset","float",1),r.constants.add("componentFieldCount","float",2),r.constants.add("lineWidthFractionFactor","float",8),r.constants.add("extensionLengthOffset","float",128),r.constants.add("componentTexWidth","float",4096),r.code.add(o`
  ...
  `),n.legacy?r.code.add(o`
    ...
    `):(r.uniforms.add("uTransformNormal_GlobalFromModel ","mat3"),r.code.add(o`
      vec3 _modelToWorldNormal(vec3 normal) {
        return uTransformNormal_GlobalFromModel * normal;
      }
    `)),n.silhouette?(t.attributes.add("normalA","vec3"),t.attributes.add("normalB","vec3"),r.code.add(o`
      vec3 worldNormal() {
        return _modelToWorldNormal(normalize(normalA + normalB));
      }
    `)):(t.attributes.add("normal","vec3"),r.code.add(o`
      vec3 worldNormal() {
    ...
    `):(t.vertex.include(e,n),r.code.add(o`
      vec3 worldFromModelPosition(vec3 position) {

...
*/
import{glsl as e}from"../views/3d/webgl-engine/core/shaderModules/interfaces.js";import{Transform as o}from"../views/3d/webgl-engine/core/shaderLibrary/Transform.glsl.js";import{ShaderBuilder as r}from"../views/3d/webgl-engine/core/shaderModules/ShaderBuilder.js";import{Slice as a}from"../views/3d/webgl-engine/core/shaderLibrary/Slice.glsl.js";import{OutputHighlight as i}from"../views/3d/webgl-engine/core/shaderLibrary/output/OutputHighlight.glsl.js";import{OutputDepth as t}from"../views/3d/webgl-engine/core/shaderLibrary/output/OutputDepth.glsl.js";import{NormalUtils as l}from"../views/3d/webgl-engine/core/shaderLibrary/shading/NormalUtils.glsl.js";import{EvaluateSceneLighting as n}from"../views/3d/webgl-engine/core/shaderLibrary/shading/EvaluateSceneLighting.glsl.js";import{HeaderComment as v}from"../views/3d/webgl-engine/core/shaderLibrary/util/HeaderComment.glsl.js";import{VertexTangent as s}from"../views/3d/webgl-engine/core/shaderLibrary/attributes/VertexTangent.glsl.js";import{Overlay as c}from"../views/3d/webgl-engine/core/shaderLibrary/terrain/Overlay.glsl.js";import{Skirts as d}from"../views/3d/webgl-engine/core/shaderLibrary/terrain/Skirts.glsl.js";import{TerrainTexture as m}from"../views/3d/webgl-engine/core/shaderLibrary/terrain/TerrainTexture.glsl.js";function g(g){const p=new r;if(p.include(v,{name:"Terrain Shader",output:g.output}),p.include(d),p.attributes.add("position","vec3"),p.attributes.add("uv0","vec2"),p.vertex.uniforms.add("proj","mat4").add("view","mat4").add("origin","vec3").add("skirtScale","float"),0===g.output){p.include(o,{linearDepth:!1}),p.include(l,g),p.include(m,g);const r=0!==g.overlayMode,i=2===g.overlayMode;r&&p.include(c,{pbrMode:3,useCustomDTRExponentForWater:!1,ssrEnabled:g.ssrEnabled,highStepCount:g.highStepCount}),i&&p.include(s,g),p.varyings.add("vnormal","vec3"),p.varyings.add("vpos","vec3"),p.vertex.uniforms.add("viewNormal","mat4"),g.receiveShadows&&p.varyings.add("linearDepth","float"),g.tileBorders&&p.varyings.add("vuv","vec2"),g.atmosphere&&(p.vertex.uniforms.add("lightingMainDirection","vec3"),p.varyings.add("wnormal","vec3"),p.varyings.add("wlight","vec3")),g.screenSizePerspective&&(p.vertex.uniforms.add("screenSizePerspective","vec4"),p.varyings.add("screenSizeDistanceToCamera","float"),p.varyings.add("screenSizeCosAngle","float")),p.vertex.code.add(e`
      void main(void) {
        vpos = position;
        ...
        ${g.tileBorders?e`vuv = uv;`:""}
        ${g.screenSizePerspective?e`
        vec3 viewPos = (view * vec4(vpos, 1.0)).xyz;
        screenSizeDistanceToCamera = length(viewPos);
        vec3 viewSpaceNormal = (viewNormal * vec4(normalize(vpos + origin), 1.0)).xyz;
        screenSizeCosAngle = abs(viewSpaceNormal.z);`:""}
        gl_Position = transformPosition(proj, view, vpos);
        ${g.receiveShadows?e`linearDepth = gl_Position.w;`:""}
        forwardTextureCoordinates(uv);
        ${r?e`setOverlayVTC(uv);`:""}
        ${i?e`forwardVertexTangent(vnormal);`:e``}
      }
    `),p.extensions.add("GL_OES_standard_derivatives"),p.extensions.add("GL_EXT_shader_texture_lod"),p.include(a,g),p.include(n,g),p.fragment.uniforms.add("camPos","vec3").add("viewDirection","vec3").add("ssaoTex","sampler2D").add("viewportPixelSz","vec4").add("opacity","float"),g.screenSizePerspective&&p.fragment.uniforms.add("screenSizePerspective","vec4"),i&&(p.fragment.uniforms.add("ovInnerWaterTex","sampler2D"),p.fragment.uniforms.add("ovOuterWaterTex","sampler2D"),p.fragment.uniforms.add("view","mat4")),p.fragment.code.add(e`
      ...
      `),g.atmosphere&&p.fragment.code.add(e`
      ...
      void main() {
        ${g.receiveShadows?e`float shadow = readShadowMap(vpos, linearDepth);`:e`float shadow = 0.0;`}
    ...
    `)}return 1!==g.output&&3!==g.output||(p.include(o,{linearDepth:!0}),p.include(t,{output:g.output}),p.include(l,g),p.varyings.add("linearDepth","float"),p.vertex.uniforms.add("nearFar","vec2"),p.vertex.code.add(e`
        void main(void) {
    ...
    `)),2===g.output&&(p.include(o,{linearDepth:!1}),p.include(l,g),p.varyings.add("vnormal","vec3"),p.varyings.add("vpos","vec3"),p.vertex.uniforms.add("viewNormal","mat4"),p.vertex.code.add(e`
        void main(void) {
          vec3 normal = getLocalUp(position, origin);
          vec2 uv = uv0;
    ...
    `)),4===g.output&&(p.include(o,{linearDepth:!1}),p.include(l,g),p.include(c,{pbrMode:0}),p.vertex.code.add(e`
          void main() {
      ...
      `),p.include(i),p.fragment.code.add(e`
        void main() {
